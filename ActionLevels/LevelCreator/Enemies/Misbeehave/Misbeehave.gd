		...
		if !parent_node.is_move_disabled:
			if parent_node.player_global_position != Vector2(0,0) and parent_node.is_on_screen():
				if parent_node.position.x - parent_node.player_global_position.x > 0:
					parent_node.position = parent_node.position.move_toward(parent_node.player_global_position, (parent_node.initial_scroll_speed / 2) * delta)
	parent_node.position.x -= parent_node.initial_scroll_speed * 1.05 * delta
