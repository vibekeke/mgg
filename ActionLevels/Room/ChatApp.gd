...
func _ready():
	...
	print("printing typing sound state ", typing_sounds)
...
func type_then_send(text_to_type):
...
func _on_SendMessageButton_button_up():
	...
	if current_script_entry['name'] == PLAYER_IDENTIFIER:
		type_then_send(current_script_entry['script'])
		#chat_sender_box.text = current_script_entry['script']
