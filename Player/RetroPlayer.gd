...
# main difference with retro player is that they are in the retro viewport
onready var retro_viewport = get_node("%Viewport")
...
# black
var original_color : Color = Color(0.0, 0.0, 0.0, 1.0)
var flash_color : Color = Color(0.0, 0.0, 0.0, 1.0)
...
func shoot(angle):
	...
	if retro_viewport == null:
		print_debug("Using retro player without retro viewport!")
	retro_viewport.add_child(_gunshot)
...
func fire_charge_shot():
	...
	if retro_viewport == null:
		print_debug("Using retro player without retro viewport")
	retro_viewport.add_child(_charge_shot)
